...
class QuadrotoriLQR_EA(LeafSystem):
    ...
    #         deltaJ, K, d[:] = self._backward_pass(xtraj, utraj, gradV, hessV, d, K, regu)
    #         # Forward rollout with line search
    #         alpha = 1.0
    #         xtraj[:], utraj[:], Jn = self._forward_rollout(xtraj, utraj, d, K, alpha)
    ...
    def _backward_pass(self, xtraj, utraj, gradV, hessV, d, K, regu):
        deltaJ = 0
        ...
        for k in range(self.num_time_steps-1)[::-1]:
            ...
            # Determine action value derivatives
            Q_xx = l_xx+ A.T @ hessV_k1 @ A
            Q_ux = l_ux + B.T @ hessV_k1 @ A
            Q_xu = Q_ux.T
            Q_uu = l_uu + B.T @ hessV_k1 @ B 

            Q_x = l_x + A.T @ gradV_k1
            Q_u = l_u + B.T @ gradV_k1
